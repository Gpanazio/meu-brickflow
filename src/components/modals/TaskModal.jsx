...
export default function TaskModal({
    ...
    isReadOnly,
    currentUser,
    users // fallback users if API fails or for initial state
}) {
    ...
    }, [modalState?.isOpen, taskState?.id]);
    ...
    return (
        <DialogContent className="bg-black border border-zinc-800 text-zinc-100 p-0 gap-0 shadow-2xl rounded-none sm:max-w-[700px]">
            <DialogHeader className="p-6 border-b border-zinc-900 flex flex-row items-center justify-between space-y-0">
                <DialogTitle className="text-lg font-black uppercase tracking-tight flex items-center gap-2">
                    ...
                    {modalState.mode === 'edit' ? taskState.title || 'Editar' : 'Novo Card'}
                </DialogTitle>
