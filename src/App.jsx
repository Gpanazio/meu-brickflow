...
import { 
  ...
  X, Check, ChevronDown, ChevronRight, Search, GripVertical, Calendar, User, Settings, Image as ImageIcon
} from 'lucide-react';
...
const Input = React.forwardRef(({ className, type, ...props }, ref) => {
  return (
    <input
      type={type}
      className={cn(
        "flex h-10 w-full rounded-md border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm shadow-sm transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-zinc-600 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-red-600 disabled:cursor-not-allowed disabled:opacity-50 text-zinc-100",
        className
      )}
      ref={ref}
      {...props}
...
const Textarea = React.forwardRef(({ className, ...props }, ref) => {
  return (
    <textarea
      className={cn(
        "flex min-h-[80px] w-full rounded-md border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm shadow-sm placeholder:text-zinc-600 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-red-600 disabled:cursor-not-allowed disabled:opacity-50 text-zinc-100",
        className
      )}
      ref={ref}
      {...props}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-50 grid w-full max-w-lg gap-4 border border-zinc-800 bg-black p-6 shadow-lg duration-200 sm:rounded-lg max-h-[90vh] overflow-y-auto custom-scrollbar">
        ...
        <button
          ...
          className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-black transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-zinc-700 focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-zinc-800 data-[state=open]:text-zinc-400"
        >
...
const Select = ({ value, onValueChange, children, name, defaultValue }) => {
  return (
    <div className="relative">
      <select 
        ...
        className="flex h-10 w-full items-center justify-between rounded-md border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm shadow-sm ring-offset-black placeholder:text-zinc-600 focus:outline-none focus:ring-1 focus:ring-zinc-700 disabled:cursor-not-allowed disabled:opacity-50 text-white appearance-none"
      >
...
const Checkbox = React.forwardRef(({ className, checked, onCheckedChange, defaultChecked, ...props }, ref) => (
  <input
    ...
    className={cn(
      "peer h-5 w-5 shrink-0 rounded-sm border border-zinc-800 ring-offset-black focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-700 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 checked:bg-white checked:text-black accent-white",
      className
    )}
    {...props}
  />
...
const DropdownMenuContent = ({ children, isOpen, align = 'start', className }) => {
  ...
  return (
    <div className={cn(
      "absolute z-50 min-w-[10rem] overflow-hidden rounded-md border border-zinc-800 bg-black p-1 text-zinc-100 shadow-md animate-in data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
...
const DropdownMenuItem = ({ children, className, onClick, ...props }) => (
  <div
    className={cn(
      "relative flex cursor-default select-none items-center rounded-sm px-3 py-2 text-sm outline-none transition-colors hover:bg-zinc-900 hover:text-zinc-100 focus:bg-zinc-900 focus:text-zinc-100 data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className
    )}
...
const DropdownMenuSeparator = ({ className, ...props }) => (
  <div className={cn("-mx-1 my-1 h-px bg-zinc-800", className)} {...props} />
);

const Avatar = ({ className, children }) => (
  <div className={cn("relative flex h-12 w-12 shrink-0 overflow-hidden rounded-full border border-zinc-800", className)}>
    {children}
  </div>
);

const AvatarImage = ({ src, className }) => (
  <img src={src} alt="Avatar" className={cn("aspect-square h-full w-full object-cover", className)} />
);

const AvatarFallback = ({ className, children }) => (
  <div className={cn("flex h-full w-full items-center justify-center rounded-full bg-zinc-900 font-medium", className)}>
    {children}
  </div>
);

const Separator = ({ className, orientation = "horizontal" }) => (
  <div className={cn("shrink-0 bg-zinc-800", orientation === "horizontal" ? "h-[1px] w-full" : "h-full w-[1px]", className)} />
);
...
function SudokuGame() {
  ...
  if (!showGame) return (
    <div className="p-6 border border-zinc-800 bg-black text-zinc-500 text-sm font-mono uppercase tracking-widest text-center">
        Procrastinação derrotada! De volta ao batente! 💪
    </div>
  );
  ...
  return (
    <div className="relative border border-zinc-800 bg-black p-6" data-testid="sudoku-game">
      {showOverlay && (
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/90 backdrop-blur-sm p-6 text-center">
          <div className="space-y-6">
            <p className="text-zinc-200 text-lg">
              O trabalho chama, mas o Sudoku é muito mais divertido. Qual é a sua escolha?
            </p>
            <div className="flex flex-col gap-3">
              <Button onClick={() => setShowOverlay(false)} className="w-full text-base">
                Estou ciente que preciso trabalhar, mas escolho jogar
              </Button>
              <Button variant="secondary" onClick={() => { setShowGame(false); setShowOverlay(false); }} className="w-full text-base">
                Obrigado por me lembrar, prefiro trabalhar
              </Button>
            </div>
          </div>
        </div>
      ...
      {showFinalOverlay && (
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/90 p-6 text-center">
          <p className="text-zinc-200 text-lg">
            Fracassar tão perto do sucesso é uma arte. Parabéns, você é um artista incompreendido! 
          </p>
        </div>
      )}

      <h3 className="mb-6 text-base font-bold uppercase tracking-widest text-zinc-500 flex items-center gap-2">
        <Sparkles className="h-5 w-5" /> Sudoku
      </h3>
      <div className="grid grid-cols-9 gap-px bg-zinc-800 border border-zinc-800 mb-6" onMouseDown={handleFirstInteraction}>
        {board.map((row, r) =>
          row.map((cell, c) => (
            <input
              key={`${r}-${c}`}
              className={`aspect-square w-full bg-black text-center text-sm font-mono outline-none focus:bg-zinc-900
                ${fixedCells[r][c] ? 'text-zinc-500' : 'text-white'}
                ${!isCellValid(board, r, c) ? 'text-red-500' : ''}
              `}
              value={cell}
      ...
      <Button variant="outline" size="sm" onClick={restart} className="w-full uppercase text-xs tracking-widest">
        Reiniciar
      </Button>
    </div>
  )
  ...
  return (
    <div className="relative inline-block">
      <button 
        ...
        className="flex items-center gap-2 rounded bg-zinc-900 px-3 py-1.5 text-xs font-medium text-zinc-400 hover:text-white transition-colors"
      >
        👤 {users.length}
      </button>
      {open && (
        <div className="absolute top-full left-0 mt-2 z-10 w-48 rounded border border-zinc-800 bg-black p-2 shadow-xl">
          {users.map((user, i) => (
            <div key={i} className="text-xs font-medium text-zinc-300 py-2 px-2 border-b border-zinc-900 last:border-0 hover:bg-zinc-900 transition-colors">
              👤 {user}
            </div>
          ))}
        </div>
...
// --- 5. LEGACY COMPONENTS REFACTORED ---
    ...
    <header className="sticky top-0 z-50 w-full border-b border-zinc-900 bg-black/95 backdrop-blur">
      <div className="container flex h-20 items-center justify-between mx-auto px-6 md:px-10">
        <div className="flex items-center gap-8">
          <div onClick={() => setCurrentView('home')} className="cursor-pointer hover:opacity-80 transition-opacity font-black text-white tracking-tighter text-2xl md:text-3xl">
             BRICKFLOW
          </div>
          ...
          <nav className="flex items-center gap-4">
            ...
            {currentProject && (
              <>
                ...
                <Button variant="ghost" className="uppercase tracking-widest text-xs font-bold text-zinc-500 hover:text-white rounded-none h-10 px-4" onClick={() => setCurrentView('project')}>{currentProject.name}</Button>
              </>
            )}
          </nav>
        </div>
              ...
              <Button variant="ghost" className="h-12 w-12 rounded-full p-0 hover:bg-zinc-900 border border-transparent hover:border-zinc-800">
                <Avatar className="h-10 w-10">
                  {currentUser?.avatar ? (
                    ...
                    <AvatarFallback className="bg-zinc-900 text-zinc-400 text-sm font-bold">{currentUser?.displayName?.charAt(0)}</AvatarFallback>
                  )}
                </Avatar>
              </Button>
            </DropdownMenuTrigger>
          ...
          <div className="w-48 border-r border-zinc-900 p-4 bg-zinc-950/30">
            <div className="space-y-1">
              <button 
                ...
                className={cn(
                  "w-full text-left px-3 py-2 rounded-md text-xs font-bold uppercase tracking-widest transition-colors",
                  activeTab === 'profile' ? "bg-zinc-800 text-white" : "text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900"
                )}
              >
                Perfil
              </button>
              ...
              <div className="space-y-8">
                  ...
                  <div className="flex items-start gap-6">
                    <Avatar className="w-24 h-24 border-2 border-zinc-800">
                      {avatarPreview ? (
                        ...
                        <AvatarFallback className="bg-zinc-900 text-zinc-500 text-2xl font-bold">{currentUser?.displayName?.charAt(0)}</AvatarFallback>
                      )}
                    </Avatar>
                    
                    <div className="space-y-3 flex-1">
                      <div className="flex gap-3">
                        <Button variant="outline" onClick={() => fileInputRef.current?.click()} className="h-9 text-xs uppercase font-bold tracking-widest">
                      ...
                      <p className="text-[10px] text-zinc-600 font-mono leading-relaxed">
                        Recomendado: 400x400px. <br/>Suporta JPG, PNG e GIF.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <h3 className="text-sm font-bold text-white uppercase tracking-widest mb-4 flex items-center gap-2">
                    <Sparkles className="w-3 h-3 text-purple-500" /> Avatares No Sense
                  </h3>
                  <div className="grid grid-cols-4 gap-3">
                    {NO_SENSE_AVATARS.map((url, idx) => (
                      <div 
                        ...
                        className={cn(
                          "aspect-square rounded-md overflow-hidden cursor-pointer border-2 transition-all hover:scale-105",
                          avatarPreview === url ? "border-red-600 opacity-100" : "border-transparent opacity-60 hover:opacity-100 hover:border-zinc-700"
                        )}
                      >
...
function LegacyModal({ modalState, setModalState, handlePasswordSubmit, handleSaveProject, handleTaskAction, allUsers, currentUser }) {
  ...
  // Logic to determine initial checked state for checkboxes
  // If editing, use existing enabledTabs. If creating, default to all.
  const isCreate = modalState.mode === 'create';
  const initialTabs = modalState.data?.enabledTabs || ['kanban', 'todo', 'files', 'goals'];
  
  // We need local state for checkboxes because FormData only gives us checked ones on submit, 
  // but we might want to control UI or validation
  
  useEffect(() => {
    setSelectedUsers(modalState.data?.responsibleUsers || []);
  }, [modalState.data]);
        ...
        <div className="p-8 max-h-[60vh] overflow-y-auto custom-scrollbar">
          {modalState.type === 'password' ? (
            ...
            <form onSubmit={(e) => {
              ...
              // Inject selected users into data
              ...
              {(modalState.type === 'project' || modalState.type === 'subProject') && (
                <>
                  <div className="space-y-3">
                    <Label className="text-xs uppercase tracking-widest text-zinc-500 font-bold">Nome do Projeto</Label>
                    <Input name="name" defaultValue={modalState.data?.name} required className="bg-zinc-950 border-zinc-800 rounded-md h-12 focus:border-white text-white text-base px-4" />
                  </div>
                  <div className="space-y-3">
                    <Label className="text-xs uppercase tracking-widest text-zinc-500 font-bold">Descrição</Label>
                    <Textarea name="description" defaultValue={modalState.data?.description} className="bg-zinc-950 border-zinc-800 rounded-md min-h-[100px] text-zinc-300 focus:border-white text-sm p-4" />
                  </div>
                            ...
                            {ALL_TABS.map(tab => (
                                <div key={tab.id} className="flex items-center gap-3 bg-zinc-900/30 border border-zinc-800 p-3 rounded-md hover:border-zinc-700 transition-colors">
                                    <Checkbox 
                                        id={`view_${tab.id}`} 
                                        name={`view_${tab.id}`} 
                                        defaultChecked={isCreate ? true : initialTabs.includes(tab.id)} 
                                        className="border-zinc-600 data-[state=checked]:bg-red-600 data-[state=checked]:border-red-600" 
                                    />
                                    <Label htmlFor={`view_${tab.id}`} className="text-xs font-bold uppercase tracking-widest text-zinc-300 cursor-pointer flex items-center gap-2">
                                        <tab.icon className="w-3 h-3 text-zinc-500" />
                                        {tab.label}
                                    </Label>
                                </div>
                            ))}
                  ...
                  <div className="grid grid-cols-2 gap-6 pt-2">
                    <div className="space-y-3">
                      <Label className="text-xs uppercase tracking-widest text-zinc-500 font-bold">Cor de Identificação</Label>
                      <Select name="color" defaultValue={modalState.data?.color || "blue"} onValueChange={()=>{}}>
                        {/* Options rendered inside native select in component */}
                      </Select>
                    </div>
                    <div className="flex items-end pb-4 gap-3">
                      <div className="flex items-center gap-3 bg-zinc-900/50 p-3 rounded-md w-full border border-zinc-800">
                        <Checkbox id="prot" name="isProtected" defaultChecked={modalState.data?.isProtected} className="border-zinc-600 data-[state=checked]:bg-white data-[state=checked]:text-black" />
                        <Label htmlFor="prot" className="text-xs text-zinc-300 cursor-pointer uppercase tracking-widest font-bold">Proteger com Senha</Label>
                      </div>
                    </div>
                  </div>
                  
                  <div className="space-y-3">
                     <Label className="text-xs uppercase tracking-widest text-zinc-500 font-bold">Senha de Acesso (Opcional)</Label>
                     <Input name="password" type="password" defaultValue={modalState.data?.password} placeholder="••••••" className="bg-zinc-950 border-zinc-800 rounded-md h-12 px-4 tracking-widest" />
                  </div>
                </>
              )}
              ...
              {modalState.type === 'task' && (
                <>
                  <div className="space-y-3">
                    <Label className="text-xs uppercase tracking-widest text-zinc-500 font-bold">Título da Tarefa</Label>
                    <Input name="title" defaultValue={modalState.data?.title} required className="bg-zinc-950 border-zinc-800 rounded-md h-14 text-lg font-bold text-white focus:border-white px-4" />
                  </div>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-3">
                        <Label className="text-xs uppercase tracking-widest text-zinc-500 font-bold">Prioridade</Label>
                        <Select name="priority" defaultValue={modalState.data?.priority || 'medium'} />
                    </div>
                    <div className="space-y-3">
                        <Label className="text-xs uppercase tracking-widest text-zinc-500 font-bold">Data de Entrega</Label>
                        <Input type="date" name="endDate" defaultValue={modalState.data?.endDate} className="bg-zinc-950 border-zinc-800 rounded-md h-12 text-sm uppercase text-zinc-300 px-4" />
                    </div>
                  </div>
                  
                  <div className="space-y-3">
                    <Label className="text-xs uppercase tracking-widest text-zinc-500 font-bold">Responsáveis</Label>
                    <div className="grid grid-cols-2 gap-2 bg-zinc-950 border border-zinc-800 p-3 rounded-md">
                      {allUsers?.map(user => (
                        <div key={user.username} className="flex items-center gap-2" onClick={() => toggleUser(user.username)}>
                          <div className={`w-4 h-4 border border-zinc-600 flex items-center justify-center cursor-pointer ${selectedUsers.includes(user.username) ? 'bg-white' : ''}`}>
                             {selectedUsers.includes(user.username) && <Check className="w-3 h-3 text-black" />}
                          </div>
                          <span className="text-sm text-zinc-300 cursor-pointer select-none">{user.displayName}</span>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <DialogFooter className="pt-8 border-t border-zinc-900 gap-4">
                <Button type="button" variant="ghost" onClick={() => setModalState({ ...modalState, isOpen: false })} className="hover:bg-zinc-900 hover:text-white text-zinc-500 rounded-md uppercase text-xs tracking-widest h-12 px-6 font-bold">Cancelar</Button>
                <Button type="submit" className="bg-white text-black hover:bg-zinc-200 rounded-md uppercase text-xs font-bold tracking-widest h-12 px-8 shadow-lg shadow-white/5">Salvar</Button>
              </DialogFooter>
            </form>
          )}
        </div>
...
function LegacyHome({ currentUser, dailyPhrase, megaSenaNumbers, projects, setModalState, handleAccessProject, handleDeleteProject }) {
  ...
  // Agregar tarefas do usuário logado
      ...
      proj.subProjects?.forEach(sub => {
        ...
        sub.boardData?.kanban?.lists?.forEach(list => {
          list.tasks?.forEach(task => {
            if (task.responsibleUsers?.includes(currentUser.username) && list.title !== 'CONCLUÍDO') {
              tasks.push({
                ...task,
    ...
    <div className="space-y-12 animate-in fade-in duration-500 pb-24">
      ...
      <div className="border-b border-zinc-900 pb-8 mb-10 flex flex-col xl:flex-row xl:items-center justify-between gap-8">
        ...
        <div className="flex flex-col">
           <h1 className="text-5xl md:text-7xl font-black text-white uppercase tracking-tight leading-none mb-3">
             Olá, <span className="text-zinc-700">{currentUser?.displayName}</span>
           </h1>
           <p className="text-sm text-zinc-500 font-mono tracking-[0.2em] uppercase">
             {currentDate}
           </p>
        </div>

        {/* Widgets na mesma linha (Direita) */}
        <div className="flex flex-col md:flex-row items-start md:items-center gap-8 xl:gap-12 border-l border-zinc-900 xl:pl-12 md:pl-8 py-2">
            ...
            <div className="flex flex-col max-w-lg">
               <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest font-bold">Sorte do Dia</span>
               </div>
               <p className="text-base md:text-lg text-zinc-300 font-light italic leading-relaxed">
                 "{dailyPhrase}"
               </p>
            </div>
            ...
            <div className="flex flex-col">
               <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest font-bold">Probabilidade</span>
               </div>
               <div className="flex gap-2">
                  {megaSenaNumbers.map(n => (
                    <div key={n} className="w-8 h-8 flex items-center justify-center border border-zinc-800 text-zinc-400 font-mono text-xs font-bold bg-zinc-950 hover:border-emerald-900 hover:text-emerald-500 transition-colors cursor-default">
      ...
      {myPendingTasks.length > 0 && (
        <div className="space-y-4">
           <div className="flex items-center gap-2 pb-2 border-b border-zinc-900">
              ...
              <h2 className="text-sm font-bold text-zinc-500 uppercase tracking-[0.2em]">Minhas Tarefas Pendentes</h2>
           </div>
           
           <div className="bg-black border border-zinc-900">
              {myPendingTasks.map((task, idx) => (
                <div key={`${task.id}-${idx}`} className="flex flex-col md:flex-row md:items-center justify-between p-4 border-b border-zinc-900 last:border-0 hover:bg-zinc-950/50 transition-colors gap-4">
                   <div className="flex flex-col gap-1">
                      <span className="text-white font-bold text-base uppercase">{task.title}</span>
                      <div className="flex items-center gap-2 text-xs text-zinc-600 font-mono">
                         <span>{task.projectName}</span>
                         <span>/</span>
                         <span>{task.subProjectName}</span>
                         <span className="text-zinc-800">|</span>
                         <span className="text-zinc-500 uppercase">{task.status}</span>
                      </div>
                   </div>
                   
                   <div className="flex items-center gap-6">
                      {task.priority === 'high' && <span className="text-[10px] uppercase font-bold text-red-600 border border-red-900/30 px-2 py-1 bg-red-950/10">Alta Prioridade</span>}
                      {task.endDate && (
                      ...
                      <Button variant="ghost" size="sm" className="h-8 text-[10px] uppercase tracking-widest text-zinc-600 hover:text-white" onClick={() => handleAccessProject({ id: task.projectId }, 'project')}>
                         Ver
                      </Button>
                   </div>
                </div>
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-end pb-4 border-b border-zinc-900">
          <h2 className="text-sm font-bold text-zinc-500 uppercase tracking-[0.2em] mb-1">Projetos Ativos</h2>
          <Button onClick={() => setModalState({ type: 'project', mode: 'create', isOpen: true })} className="bg-white hover:bg-zinc-200 text-black h-10 px-6 text-xs uppercase font-bold tracking-widest rounded-none shadow-none"><Plus className="mr-2 h-4 w-4" /> Novo Projeto</Button>
        </div>

        {activeProjects.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 opacity-20">
            <h1 className="text-6xl font-black text-zinc-800 uppercase tracking-tighter">VAZIO</h1>
            <p className="text-zinc-600 font-mono uppercase tracking-widest mt-4">Nenhum projeto iniciado</p>
          </div>
        ) : (
            ...
            {activeProjects.map(project => {
              ...
              return (
                <div 
                  ...
                  className="group relative aspect-video border-r border-b border-zinc-900 bg-black hover:bg-zinc-950 transition-all cursor-pointer p-8 flex flex-col justify-between"
                >
                  <div className="flex justify-between items-start w-full">
                    <div className={`w-3 h-3 ${colors.bg}`}></div>
                    {project.isProtected && <Lock className="w-4 h-4 text-zinc-800" />}
                  </div>
                  ...
                  <div className="space-y-3 relative z-10">
                    ...
                    <p className="text-zinc-500 text-xs font-mono leading-relaxed line-clamp-2 uppercase tracking-wide">
                      {project.description || "SEM DESCRIÇÃO"}
                    </p>
                  </div>

                  <div className="flex justify-between items-end opacity-40 group-hover:opacity-100 transition-opacity">
                     <span className="text-[10px] text-zinc-600 font-mono uppercase tracking-widest font-bold">{project.subProjects?.length || 0} ÁREAS</span>
                     
                     <DropdownMenu>
                        <DropdownMenuTrigger asChild onClick={e => e.stopPropagation()}><Button variant="ghost" size="icon" className="h-8 w-8 text-zinc-700 hover:text-white rounded-none"><MoreVertical className="h-4 w-4" /></Button></DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="bg-black border-zinc-800 rounded-none">
                          <DropdownMenuItem onClick={e => { e.stopPropagation(); setModalState({ type: 'project', mode: 'edit', isOpen: true, data: project }); }} className="text-xs uppercase tracking-widest h-10 cursor-pointer font-medium">Editar</DropdownMenuItem>
                          <DropdownMenuItem className="text-red-900 focus:text-red-600 focus:bg-zinc-900 text-xs uppercase tracking-widest cursor-pointer h-10 font-medium" onClick={e => { e.stopPropagation(); handleDeleteProject(project); }}>Eliminar</DropdownMenuItem>
                        </DropdownMenuContent>
                     </DropdownMenu>
                  </div>
                </div>
    ...
    <div className="space-y-10 animate-in slide-in-from-bottom-4 duration-500 pb-24">
      <div className="flex flex-col gap-8 border-b border-zinc-900 pb-8">
        ...
        <div>
          ...
          <p className="text-zinc-500 text-sm font-mono uppercase tracking-widest mt-4 max-w-2xl leading-relaxed">{currentProject.description}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-zinc-900 border border-zinc-900">
          ...
          return (
            <div key={sub.id} onClick={() => handleAccessProject(sub, 'subproject')} className="group cursor-pointer bg-black hover:bg-zinc-950 transition-colors p-8 flex flex-col justify-between h-64">
              <div className="flex justify-between items-start">
                 <FolderOpen className={`h-6 w-6 ${colors.text} opacity-50 group-hover:opacity-100 transition-opacity`} />
                 <DropdownMenu>
                  <DropdownMenuTrigger asChild onClick={e => e.stopPropagation()}><Button variant="ghost" size="icon" className="h-8 w-8 text-zinc-800 hover:text-white rounded-none"><MoreVertical className="h-4 w-4" /></Button></DropdownMenuTrigger>
                  <DropdownMenuContent align="end" className="bg-black border-zinc-800 rounded-none">
                    <DropdownMenuItem onClick={e => { e.stopPropagation(); setModalState({ type: 'subProject', mode: 'edit', isOpen: true, data: sub }); }} className="text-xs uppercase tracking-widest h-10 cursor-pointer font-medium">Editar</DropdownMenuItem>
                    <DropdownMenuItem className="text-red-900 focus:text-red-600 text-xs uppercase tracking-widest h-10 cursor-pointer font-medium" onClick={e => { e.stopPropagation(); handleDeleteProject(sub, true); }}>Excluir</DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
              </div>
              <div>
                 <CardTitle className="text-2xl font-bold uppercase tracking-tight text-white mb-2">{sub.name}</CardTitle>
                 <span className="text-zinc-600 text-xs font-mono uppercase tracking-widest line-clamp-2 leading-relaxed">{sub.description || "---"}</span>
              </div>
            </div>
          );
        })}
        <div 
          ...
          className="bg-black flex flex-col items-center justify-center cursor-pointer group hover:bg-zinc-900/30 transition-colors h-64"
        >
          <Plus className="h-8 w-8 text-zinc-800 group-hover:text-zinc-500 mb-3 transition-colors" />
          <span className="text-xs font-mono uppercase tracking-widest text-zinc-800 group-hover:text-zinc-500 font-bold">Adicionar Área</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-6rem)]">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 border-b border-zinc-900 pb-6 gap-6">
        <div className="flex items-center gap-6">
          <Button variant="ghost" size="sm" onClick={() => setCurrentView(currentSubProject ? 'project' : 'home')} className="text-zinc-500 hover:text-white uppercase text-xs font-bold tracking-widest rounded-none px-0"><ArrowLeft className="mr-2 h-4 w-4" /> Voltar</Button>
          <h2 className="text-3xl md:text-4xl font-black text-white uppercase tracking-tight">{entityName}</h2>
        </div>
        <div className="flex gap-6 border-b border-transparent">
            {/* Custom Tabs implementation for header */}
      ...
      <div className="flex-1 overflow-hidden relative bg-black">
        <div className="absolute inset-0 overflow-auto pr-4 pb-10">
              ...
              {data.lists ? data.lists.map(list => (
                <div key={list.id} className="w-80 flex flex-col h-full bg-black border-r border-zinc-900"
                  ...
                  <div className="p-6 border-b border-zinc-900 flex justify-between items-center bg-zinc-950/30">
                    <span className="font-bold text-xs uppercase tracking-[0.2em] text-zinc-400">{list.title}</span>
                    <span className="text-zinc-600 text-xs font-mono font-bold">{(list.tasks?.length ?? 0).toString().padStart(2, '0')}</span>
                  </div>
                  <div className="flex-1 p-4 space-y-4 overflow-y-auto custom-scrollbar bg-black">
                    {list.tasks?.map(task => (
                      <div key={task.id} draggable onDragStart={(e) => handleDragStart(e, task, 'task', list.id)}
                            ...
                            className={`bg-zinc-950 border border-zinc-900 hover:border-zinc-700 cursor-grab active:cursor-grabbing p-5 group transition-all shadow-sm`}>
                        <div className="flex justify-between items-start mb-3">
                          <span className="text-sm font-bold text-zinc-200 group-hover:text-white transition-colors uppercase leading-tight tracking-tight">{task.title}</span>
                          {task.priority === 'high' && <div className="h-1.5 w-1.5 bg-red-600 shrink-0 mt-1" />}
                        </div>
                        <div className="flex items-center justify-between pt-3 border-t border-zinc-900/50 mt-1">
                          ...
                          {task.endDate && <span className="text-[10px] text-zinc-500 font-mono font-bold">{new Date(task.endDate).toLocaleDateString().slice(0,5)}</span>}
                        </div>
                    </div>
                  ))}
                  <Button variant="ghost" className="w-full border border-dashed border-zinc-900 text-zinc-600 hover:text-white hover:bg-zinc-950 rounded-none h-12 uppercase text-xs tracking-widest font-bold"
            ...
            <div className="max-w-5xl mx-auto space-y-12 pt-4">
              {data.lists ? data.lists.map(list => (
                <div key={list.id} className="space-y-0">
                  <h3 className="text-xs font-bold text-zinc-500 uppercase tracking-[0.3em] mb-4 pl-4 border-l-4 border-red-600">{list.title}</h3>
                  <div className="bg-black border-t border-zinc-900">
                    {list.tasks?.map(task => (
                      <div key={task.id} className="p-4 flex items-center gap-5 border-b border-zinc-900 hover:bg-zinc-950/50 transition-colors group">
                        <Checkbox checked={list.title === 'Concluído'} className="border-zinc-800 rounded-none w-5 h-5" />
                        <div className="flex-1 cursor-pointer" onClick={() => setModalState({ type: 'task', mode: 'edit', isOpen: true, data: task, listId: list.id })}>
                          <p className="text-base font-medium text-zinc-300 group-hover:text-white transition-colors uppercase tracking-tight">{task.title}</p>
                        </div>
                        <div className="opacity-0 group-hover:opacity-100 flex items-center gap-6">
                          <span className="text-xs font-mono uppercase text-zinc-600 font-bold tracking-wider">{task.priority === 'high' ? 'ALTA' : task.priority === 'medium' ? 'MÉDIA' : 'BAIXA'}</span>
                          <Button variant="ghost" size="icon" className="h-10 w-10 text-zinc-700 hover:text-red-600 hover:bg-transparent rounded-none" 
                    ...
                    <Button variant="ghost" className="w-full text-xs text-zinc-500 hover:text-white justify-start h-14 px-5 uppercase tracking-widest rounded-none hover:bg-zinc-950 font-bold" onClick={() => setModalState({ type: 'task', mode: 'create', isOpen: true, data: { listId: list.id } })}>
                      <Plus className="h-4 w-4 mr-3" /> Inserir Novo Item na Lista
                    </Button>
                  </div>
                </div>
          ...
          {currentBoardType === 'files' && (
            <div 
              className={`min-h-[500px] relative transition-all duration-300 p-0 ${isFileDragging ? 'bg-zinc-950 border-2 border-dashed border-red-900' : ''}`}
            ...
            >
              {(isFileDragging || isUploading) && (
                <div className="absolute inset-0 flex items-center justify-center z-50 bg-black/90 backdrop-blur-sm">
                  {isUploading ? (
                     <div className="animate-pulse text-white text-sm uppercase tracking-widest font-bold">Carregando arquivos...</div>
                  ) : (
                     <div className="text-center animate-pulse">
                        <Upload className="w-12 h-12 text-white mx-auto mb-6" />
                        <p className="text-white font-mono text-sm uppercase tracking-[0.5em] font-bold">Solte para Upload</p>
                     </div>
                  )}
                </div>
              )}
              ...
              <div className="flex justify-between items-center bg-zinc-950 p-8 border-b border-zinc-900 mb-8">
                <div>
                  <h3 className="text-3xl font-black text-white uppercase tracking-tighter">Arquivos</h3>
                  <p className="text-zinc-600 text-xs font-mono uppercase tracking-widest mt-2">Arraste arquivos ou clique no botão</p>
                </div>
                <div className="relative">
                  <Input 
                    type="file" 
                  ...
                  />
                  <Button className="bg-white text-black hover:bg-zinc-200 uppercase tracking-widest text-xs font-bold rounded-none h-12 px-8">
                    <Upload className="mr-3 h-4 w-4" /> Fazer Upload
                  </Button>
                </div>
              </div>
              
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-px bg-zinc-900 border border-zinc-900">
                {(files || []).map(file => (
                  <div key={file.id} className="bg-black hover:bg-zinc-950 transition-all group relative aspect-square flex flex-col items-center justify-center p-6">
                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                        <Button size="icon" variant="ghost" className="h-8 w-8 text-zinc-600 hover:text-red-600 rounded-none" onClick={() => handleDeleteFile(file.id)}>
                    ...
                    <p className="text-xs text-zinc-400 font-mono truncate w-full text-center group-hover:text-white transition-colors px-2">{file.name}</p>
                    <p className="text-[10px] text-zinc-600 uppercase tracking-widest mt-2 font-bold">{formatFileSize(file.size)}</p>
                  </div>
                ))}
              </div>
            </div>
...
export default function App() {
  ...
  const [allUsers, setAllUsers] = useState([
    ...
    { 
        ...
        displayName: 'Fran', 
        color: 'purple',
        avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=800&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NHx8YXZhdGFyfGVufDB8fDB8fHww' 
    } 
  ]);
  ...
  const [modalState, setModalState] = useState({
    type: null, isOpen: false, data: null, mode: 'create'
  });
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      try {
        ...
        setIsLoggedIn(true);
      } catch (e) { localStorage.removeItem('brickflow-current-user'); }
    }
    ...
    // Mock Projects load
    const savedProjects = localStorage.getItem('brickflow-projects-mock');
    if (savedProjects) {
        setProjects(JSON.parse(savedProjects));
    } else {
        // Init with some dummy data for "My Tasks" demo if empty
        const initialMock = [{
            id: 'proj-1', name: 'Projeto Demo', description: 'Demonstração de tarefas', color: 'blue', isProtected: false, createdBy: 'admin',
            subProjects: [{
                id: 'sub-1', name: 'Geral', description: 'Tarefas gerais', createdBy: 'admin',
                enabledTabs: ['kanban', 'todo', 'files'],
                boardData: {
                    kanban: { lists: [{ id: 'k1', title: 'BACKLOG', tasks: [{ id: 't1', title: 'Tarefa Exemplo Admin', priority: 'high', responsibleUsers: ['admin'] }] }] }
                }
            }]
        }];
  ...
  useEffect(() => {
    if(projects.length > 0) {
        localStorage.setItem('brickflow-projects-mock', JSON.stringify(projects));
    }
  }, [projects]);
        ...
        const updateEntity = (entity) => {
            ...
            if (action === 'save') {
                if (isEdit) {
                    ...
                    board.lists = board.lists.map(l => l.id === listId ? { ...l, tasks: [...l.tasks, { id: generateId('task'), ...data, responsibleUsers: [] }] } : l);
                }
  ...
  const globalStyles = `
    @layer base {
      :root {
        --background: 0 0% 0%; --foreground: 0 0% 90%;
        --card: 0 0% 2%; --card-foreground: 0 0% 98%;
        --popover: 0 0% 2%; --popover-foreground: 0 0% 98%;
        --primary: 356 85% 55%; --primary-foreground: 0 0% 100%;
        --secondary: 0 0% 9%; --secondary-foreground: 0 0% 90%;
        --muted: 0 0% 9%; --muted-foreground: 0 0% 50%;
        --accent: 0 0% 15%; --accent-foreground: 0 0% 100%;
        --destructive: 0 62.8% 30.6%; --destructive-foreground: 0 0% 98%;
        --border: 0 0% 9%; --input: 0 0% 9%; --ring: 356 85% 55%;
        --radius: 0px;
      }
      * { border-color: hsl(var(--border)); }
      body { background-color: hsl(var(--background)); color: hsl(var(--foreground)); font-family: system-ui, sans-serif; }
        ...
        <Card className="w-full max-w-sm border-zinc-900 bg-black shadow-2xl rounded-none">
          <div className="p-8 space-y-6">
            ...
            <form onSubmit={(e) => {
                ...
                handleLogin(fd.get('username'), fd.get('pin'));
            }} className="space-y-4">
