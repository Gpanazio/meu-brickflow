...
import './App.css'; // Importa os estilos extraídos
...
function useFiles(currentProject, currentSubProject, currentUser) {
  ...
  useEffect(() => {
    // Aqui você pode implementar o fetch real do Supabase se desejar
    // Por enquanto, mantemos vazio ou carregamos mock
  }, [currentSubProject]);
        ...
        <div className="p-8 max-h-[60vh] overflow-y-auto custom-scrollbar">
          {modalState.type === 'password' ? (
            ...
            <form onSubmit={(e) => {
              ...
              <DialogFooter className="pt-8 border-t border-zinc-900 gap-4"><Button type="button" variant="ghost" onClick={() => setModalState({ ...modalState, isOpen: false })} className="hover:bg-zinc-900 hover:text-white text-zinc-500 rounded-md uppercase text-xs tracking-widest h-12 px-6 font-bold">Cancelar</Button><Button type="submit" className="bg-white text-black hover:bg-zinc-200 rounded-md uppercase text-xs font-bold tracking-widest h-12 px-8 shadow-lg shadow-white/5">Salvar</Button></DialogFooter>
            </form>
          )}
        </div>
      </DialogContent>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (savedUser) { try { const userData = JSON.parse(savedUser); setCurrentUser(userData); setIsLoggedIn(true); } catch (e) { localStorage.removeItem('brickflow-current-user'); } }
    ...
    if (savedProjects) { setProjects(JSON.parse(savedProjects)); } else {
        const initialMock = [{ id: 'proj-1', name: 'Projeto Demo', description: 'Demonstração de tarefas', color: 'blue', isProtected: false, createdBy: 'admin', subProjects: [{ id: 'sub-1', name: 'Geral', description: 'Tarefas gerais', createdBy: 'admin', enabledTabs: ['kanban', 'todo', 'files'], boardData: { kanban: { lists: [{ id: 'k1', title: 'BACKLOG', tasks: [{ id: 't1', title: 'Tarefa Exemplo Admin', priority: 'high', responsibleUsers: ['admin'] }] }] } } }] }];
        setProjects(initialMock);
    }
  }, []);

  useEffect(() => { if(projects.length > 0) localStorage.setItem('brickflow-projects-mock', JSON.stringify(projects)); }, [projects]);

  const handleLogin = (username, pin) => {
    const user = allUsers.find(u => u.username === username && u.pin === pin);
    if (user) { setCurrentUser(user); setIsLoggedIn(true); localStorage.setItem('brickflow-current-user', JSON.stringify(user)); } else { alert("Credenciais inválidas! (Tente: admin / 1234)"); }
  };
